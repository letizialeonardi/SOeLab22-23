...
int main (int argc, char **argv)
{
   ...
   pipe_t *pipePadreFiglio;		/* array di pipe di comunicazione/sincronizzazione fra padre e figlio. NOTA BENE: questa sincronizzazione potrebbe esser
  e fatta tramite l'invio di segnali da parte del padre ai figli */
   int j; 				/* contatori */
   int fd; 				/* file descriptor */
   int pidFiglio, status, ritorno;	/* per valore di ritorno figli */
   char c,ch; 				/* carattere per leggere dal file da parte dei figli e dalle pipe da parte del padre*/
   char token='v'; 			/* carattere che serve per sincronizzare i figli: IN QUESTA SOLUZIONE NON IMPORTA IL VALORE */
	...
	/* il padre istalla il gestore handler per il segnale SIGUSR1 */
	...
	for (i=0;i<N;i++){
		...
		{ 	/* codice figlio */
			...
			//printf("DEBUG: Sono il figlio %d di indice %d e ho finito di leggere il file %s\n", getpid(), i, argv[i+1]); 
        		exit(0); //NON SI ESEGUIRA' MAI QUESTO EXIT!
		}
	} /* fine for */
