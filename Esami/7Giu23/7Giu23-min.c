/* Soluzione parte C esame del 7 Giugno 2023: versione che calcola il minimo */
#include <stdio.h> 
#include <unistd.h> 
#include <stdlib.h> 
#include <sys/wait.h> 
...
int main(int argc, char **argv) 
{
	...
	char ch;		/* variabile usata dal padre per leggere gli ultimi caratteri delle linee inviati dai figli */
	char chMin;		/* variabile per tenere traccia del carattere di codice ASCII minimo */
	char chControllo;	/* variabile per inviare indicazione ai figli */
	int indice; 		/* usata dal padre per tenere traccia dell'indice del figlio che ha calcolato il minimo */
	 	...
	 	while (read(fd, &linea[j], 1)) 
		{
			if (linea[j] == '\n') /* se siamo a fine linea */
			{
				...
				if (chControllo =='S') 
				{
					...
					linea[j+1]='\0';
					printf("Sono il figlio di indice %d e pid %d e ho trovato una linea con un ultimo carattere '%c' con codice ASCII minore degli altri nel file %s. Ecco la linea:\n%s", n, getpid(), linea[j-1], argv[n+1], linea);
...
while(!finitof())
{
	chMin=127; /* ATTENZIONE: il valore del minimo, va resettato per ogni insieme di valori letti dal padre */
	for (n=0; n < N; n++) 
	{
		...
		finito[n] = (read(pipe_fp[n][0], &ch, 1) != 1);  /* nel caso la read torni un valore diverso da 1, il corrispondente valore di finito viene settato a 1 dato che la condizione e' vera! */
