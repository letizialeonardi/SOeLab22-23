...
int main(int argc, char **argv)
{
	...
	/* nome indicato nel testo */
        ...
        for (n=0; n < N; n++)
        {
			...
			for (j=0; j<S.lung_linea+1; j++)
				S.linea_letta[j]=buffer[j];
			/* N.B.: non si puo' usare strcpy dato che buffer NON e' una stringa; in alternativa al for andava bene usare sia memcpy(S.linea_letta, buffer, S.lung_linea+1) che strncpy con gli stessi parametri */
			/* ATTENZIONE CHE SE S.lung_linea rappresentasse la lunghezza CON il terminatore, nel for precedente o nel codice alternativo sopra indicato si dovrebbe usare solo S.lung_linea */
			...
			if ((int)((status >> 8) & 0xFF) != 0) /* in questo caso c'e' stato un errore nel nipote */
				ritorno = -1;
			/* il figlio ritorna la lunghezza della linea inviata compreso il terminatore */
			else
        ...
        for (n=0; n<N; n++)
        {
         /* si legge la struttura inviata  dal figlio i-esimo */
            ...
            if (nr != 0)
            {
                ...
                S.linea_letta[S.lung_linea+1]='\0'; /* dato che lung_linea rappresenta la lunghezza della linea, NON compreso il terminatore, per avere l'indice giusto dobbiamo incrementare di 1 questo valore */
